...
The checked-in registrations are templates. Their zero digests are placeholders
and cannot establish a valid runtime profile. `kustomization.yaml` includes only
the two reference-only Agents. It excludes the registrations and Task so an
ordinary apply does not register placeholder authority or start inference.

## Register the backends

Use the [external runtime contract](../../website/docs/guides/bring-your-own-agent-runtime.md)
to configure each service and its authentication. Edit copies of
[agentruntime-agentkit.yaml](agentruntime-agentkit.yaml) and
[agentruntime-foundry.yaml](agentruntime-foundry.yaml) with the actual endpoint,
runtime identity, profile digests, and advertised protocol limits.
...
Both profiles use `workspaceIntent: read`, `proxyCredentialRole:
operator-managed`, `proxyCredentialScope: external-runtime`, and
`resourceClass: external`. The model must match the baked configuration. Keep
the empty `mcpPolicy` in the templates. The runtime's `/v2/capabilities` must
...
must match the supervisor's canonical profile and `/v2/capabilities` response.
...
Set `ORKA_ACP_CONTROLLER_EPOCH` to the current controller epoch before starting
either supervisor. Its operator must handle epoch changes as described in the
external runtime contract. A stale supervisor cannot accept new Tasks. Do not
replace a Foundry lifetime or delete its ledger while remote ownership remains
unresolved.
...
controller. Adjust the Service namespaces in the registration endpoints and
Secret annotations together. The following example assumes the controller
watches `orka-system`; substitute your deployment's watched namespace.

```bash
FIBEY_CONTEXT=sertac-aks
FIBEY_NAMESPACE=orka-system

# Apply your edited registrations only after the Services and Secrets exist.
kubectl --context="$FIBEY_CONTEXT" -n "$FIBEY_NAMESPACE" apply -f /path/to/configured-registrations/
