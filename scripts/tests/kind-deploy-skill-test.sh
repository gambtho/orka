...
ruby -ropenssl - "${work}/valid.crt" "${work}/tls.key" "${work}/expired.crt" "${work}/ca-bundle.crt" <<'RUBY'
...
export TEST_VALID_CERT TEST_EXPIRED_CERT TEST_CA_BUNDLE
  ...
  *'get secret orka-admission-tls'*)
    ...
    if [[ "${TEST_TLS_PRESENT}" == 1 ]]; then
      if [[ "$*" == *'-o json' ]]; then
        ...
        jq -n --arg tls "${tls_cert}" --arg ca "${ca_cert}" \
          '{data: {"tls.crt": $tls, "ca.crt": $ca}}'
      else
...
for tls_mode in expired expired-ca missing malformed; do
  if TEST_TLS_MODE="${tls_mode}" run_script --cluster skill-test; then
    echo "accepted ${tls_mode} admission certificate" >&2
    exit 1
  fi
  ...
  if grep -Eq '^(make|docker|skopeo) |^kubectl (.* )?(create|apply|delete) ' "${TEST_LOG}"; then
    echo 'invalid admission certificates caused build or mutation commands' >&2
    exit 1
  fi
done
